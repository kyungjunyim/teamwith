			...
			<div class="modal-dialog modal-md modal-dialog-centered"style="max-width: 420px;" 
				...
				<div class="modal-content">
					...
					<div class="modal-body modal_text" style="text-align: center;">
						<span class="text_blue">팀위드는 팀 정보인</span><br> <span
							class="text_orange">지역, 관심 분야, 요구 기술, 역할</span><span
							class="text_blue">에 따라<br> 어울리는 팀원을 추천해드립니다.<br>
						<br> 더욱 정확한 추천을 위해
						</span><br>
						<span class="text_orange"> 팀의 정보를 업데이트</span><span
		...
		<div class="row coworker_row">
			<c:forEach items="${recommendedMemberList }" var="recommendedMember">
				<c:if test="${teamInfo.memberId != recommendedMember.memberId }">
					<div id="teamMember" class="coworker_col"
						onclick="location = '/polog/${recommendedMember.memberId }'">
						<div>
							<img src="${recommendedMember.memberPic }"
								class="team_detail_img_sm">
						</div>
						<div>
							<label class="coworker_text">${recommendedMember.memberName }님</label>
						</div>
						<div>
							<label class="coworker_text text_orange">일치율
								${fn:substring(recommendedMember.rate, 0, 4) }%</label>
						</div>
						<div>
							<label class="coworker_text" style="font-size: 14px;">${applicationScope.roleList[recommendedMember.roleId] }</label>
						</div>
					</div>
				</c:if>
			</c:forEach>
