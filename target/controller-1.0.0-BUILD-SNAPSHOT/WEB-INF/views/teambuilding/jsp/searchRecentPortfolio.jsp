...
<body>
	...
	<!-- 최근 포폴 목록 보여주기 -->
	<c:forEach items="${recentPortfolioList }" var="portfolio" varStatus="i">
	<c:if test="${(i.index mod 2) eq 0  }">
	<div class="row portfolio_search_row_whole">
	</c:if>
	<form action="/portfolio/${fn:split(portfolio.portfolioId,'-')[1]}" method="get" id="${portfolio.portfolioId }">
		<div class="col-xs-6 portfolio_search_content_row" id="${portfolio.portfolioId }_box" onclick="$('#${portfolio.portfolioId }').submit()">
			...
			<div class="row portfolio_search_content text-truncate">
				<label class="portfolio_search_intro text-truncate">${portfolio.portfolioIntro }</label>
			</div>
		</div>
	</form>
